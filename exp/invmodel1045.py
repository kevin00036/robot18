...
from data_many import RealData
import sys
...
data = [(
    ...
    np.concatenate([data[i+1][1]]), #data[i+1][1]
    discrete_action(data[i-1][2]), 
...
class LinearNet(torch.nn.Module):
	def __init__(self, D_in1, D_in2, D_out):
		super().__init__()
		self.linear = torch.nn.Linear(4, N)
		
		
		self.linear2 = torch.nn.Linear(N, N)
		#self.linear3 = torch.nn.Linear(2*N+7, 7)

		#self.linear4 = torch.nn.Linear(N+7, D_in1)
		
		self.linear3 = torch.nn.Linear(N, 7)
	def forward(self, obs1, obs2, act, act_prev):

	
		obs1 = [r for r in torch.split(obs1, 2, dim=1)]
		obs1 = torch.stack(obs1, dim=1)

		#obs1 = torch.sum(obs1, dim=0)
		
		
		obs2 = [r for r in torch.split(obs2, 2, dim=1)]
		obs2 = torch.stack(obs2, dim=1)
		#obs2 = torch.sum(obs2, dim=0)
		
		obs = torch.cat([obs1, obs2-obs1], dim=2) #(32, 6, 4)
		...
		"""
		"""
		pred1 = torch.cat([obs1, obs2, one_hot_embedding(act2,7)], dim=1)
...
for epoch in range(30000):
	...
	running_corrects = 0
	for i, (dt, obs, act, obs_next, act_prev) in enumerate(train_loader):
		...
		train_losses.append((float(loss1))) #, float(loss3)))
		running_corrects += torch.sum(preds == y.data)
	print(running_corrects.numpy()/train_num)
		
	with torch.no_grad():
		running_corrects = 0
		for i, (dt, obs, act, obs_next, act_prev) in enumerate(val_loader):
			# print(dt, obs, act, obs_next)
			# Forward pass: Compute predicted y by passing x to the model
			...
			val_losses.append((float(loss1))) #, float(loss3)))
