	...
	for epoch in range(300):
		...
		for mode in ['train', 'val']:
			...
			for i, (dt, obs, act, obs_next, v) in enumerate(loader):
				...
				x2 = obs_next
				y = act.long()
				y_pred, obs_pred1, obs_pred2 = model(x1, x2, act)
				_, preds = torch.max(y_pred.data, 1)
	
				loss = criterion(y_pred, y)
				
				loss1 = criterion2(obs_pred1, obs_next)
				loss2 = criterion2(obs_pred2, obs_next)
				final_loss = loss + w1*loss1 + w2*loss2
			...
			print('{}:\t Loss = {:.4f},\t Acc = {:.4f},\t Loss Con1 = {:.4f},\t Loss Con2 = {:.4f}'.format(mode, loss, acc, loss1, loss2))
