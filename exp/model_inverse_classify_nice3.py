...
class LinearNet(torch.nn.Module):
	def __init__(self):
		...
		self.linear12 = torch.nn.Linear(hid_dim, hid_dim)

		self.linear21 = torch.nn.Linear(14, hid_dim)
	...
	def forward_obs(self, obs):
		...
		res = F.relu(self.linear12(res))
		return res

	def forward_diff(self, obs):
		res = F.relu(self.linear21(obs))
	...
	def forward(self, obs1, obs2):
		...
		"""
		obs = self.forward_obs(obs1)
		diff = self.forward_diff(obs2-obs1)
		pred = self.linear3(torch.cat([obs, diff], dim=1)) #(32, 6, 7)
		# pred = pred.sum(dim=1)
